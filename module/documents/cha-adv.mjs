...
export class CharacterAdvancement extends FormApplication {
    ...
    async getData (options) {
        if (!this.data) {
            ...
            this.data.feats.awail = temp_feat_list.filter(item => (item.data.type === 'feature' || item.data.type === 'spell') && ((!(id_array.includes(item.data._id) || name_array.includes(item.data.name)) || item.data.data.level.max !== null || item.data.data.level.current === item.data.data.level.max)))
