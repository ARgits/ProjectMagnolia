...
export class ARd20Item extends Item {
  ...
  prepareDerivedData () {
    ...
    if (itemData.type === "feature") {
      ...
      data.level.current = (itemData.flags.core?.sourceId || this.isOwned) ? game.items.get(itemData.flags.core?.sourceId)?.data.data.level.current || data.level.current : data.level.current || 0
