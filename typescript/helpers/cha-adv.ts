...
> {
  ...
  async InitilizeData() {
    ...
    for (let v of startingData.feats.learned) {
      console.log(v);
      if (v.type === "feature")
        v.data.source.value.forEach((val: string) => {
          console.log(val);
  ...
  async getRacesList(
    ...
    pack_list.forEach((item) => {
      if (item.type === "race") {
        let raceItem = <RaceItem>{ ...item, chosen: false };
  ...
  async getFeaturesList(
    ...
    pack_list.forEach((item) => {
      if (item.type === "feature") {
        let FeatureItem = <FeatureItem>{ ...item, currentXP: 0, isEq: false, isXP: false };
        feat_pack_list.push(FeatureItem);
      }
    });
    let feat_folder_list: FeatureItem[] = [];
    folder_list.forEach((item) => {
      if (item.type === "feature") {
        let FeatureItem = <FeatureItem>{ ...item, currentXP: 0, isEq: false, isXP: false };
    ...
    let learnedFeatures: FeatureItem[] = [];
    this.object.itemTypes.feature.forEach((item) => {
      if (item.data.type === "feature") {
        let FeatureItem = <FeatureItem>{ ...item.data, currentXP: 0, isEq: false };
        learnedFeatures.push(FeatureItem);
      }
    });
    return { temp_feat_list, learnedFeatures };
  }
  async getData(): Promise<CharacterAdvancementFormAppData> {
    ...
    count.feats.all = 0;
    count.feats.all = Object.values(count.feats).reduce(function (a: number, b: number) {
      return a + b;
    }, 0);
    /*
  ...
  async _updateObject(_event: Event, formData: {}): Promise<void> {
    ...
    console.log(obj);
    const feats_data: { new: FeatureItem[]; exist: FeatureItem[] } = {
      new: [],
      exist: [],
    };
